    ...
    try:
        if req.type == "url" or URL_PATTERN.match(req.query):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"Background worker unavailable: {celery_err}. Please try again shortly.",
                )
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Database unavailable: {e}. Please try again shortly.",
        )
...
async def get_search_status(task_id: str):
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail=f"Worker status unavailable: {e}",
        )
